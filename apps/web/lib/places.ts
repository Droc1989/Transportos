...
type Row = ResolvedPlace & { match?: 'exact' | 'alias' | 'prefix' | 'similar'; kind?: string };
 ...
 * - text cu o singură potrivire sigură (nume exact sau denumire uzuală): acea localitate;
...
export async function resolvePlace(
  ...
  const strong = rows.filter((r) => r.match === 'exact' || r.match === 'alias');
